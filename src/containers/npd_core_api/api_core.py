...
core_api_url = 'npd_core_api:8080'
pred_api_url = 'npd_pred_api:8088'
train_api_url = 'npd_train_api:8888'
is_training = False
...
#Initialisation du dictionnaire des utilisateurs connus
users = {}
...
async def api_check():
    ...
    """
    try:
        async with aiofiles.open(users_init_path, 'r') as file:
            return True
...
async def user_to_dataset(ident: str, prenom: str, nom: str, email: str, motdepasse: str):
   ...
   Args:
    ...
    Aucune exception n'est levée en cas de succès.
    """
    # Utilisation des variables globales users et users_init
    global users
    global users_init
...
async def check_connection():
    """
    Description:
    Cette route vérifie l'état de l'API en vérifiant la présence du fichier "./$dataFolder/BDD/NPD_Buffer.csv". Si le fichier est présent, l'API renvoie un message indiquant que tout fonctionne correctement. Sinon, une exception est levée pour signaler un problème.
    ...
    if image.filename.endswith(('.jpg', '.jpeg', '.JPG', '.JPEG')):
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                headers = {'Authorization': 'Basic YWRtaW46NGRtMU4='}
...
@app.post("/opinion/")
async def add_opinion(username: str = Depends(get_current_user), image: str = Form(...), opinionPrediction: str = Form(...), opinionDuree: str = Form(...)):
    """
    Description:
